...
import { CloudrunV2ServerGroupTransformer } from './serverGroupTransformer.service';

describe('CloudrunV2ServerGroupTransformer', function () {
  it('normalizes server group name', async () => {
    const transformer = new CloudrunV2ServerGroupTransformer();
    ...
    } as unknown) as ICloudrunServerGroup;
  ...
  it('does not normalize server group if different account', async () => {
    ...
    const sgMgrRef = { account: ACCOUNT, location: LOCATION, name: SG_MANAGER };
    const sgAccount: ICloudrunServerGroup = ({
      account: ACCOUNT,
      name: 'myservergroup',
      region: LOCATION,
      serverGroupManagers: [sgMgrRef],
